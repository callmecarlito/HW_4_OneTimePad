...
int main(int argc, char *argv[]){
    ...
    if(port_number == 0){
        fprintf(stderr, "otp_dec: Invalid port number entered\n");
        exit(1);
    } 
    //set up address struct for otp_dec_d
    memset((char*)&server_addr, '\0', sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(port_number);
    server_info = gethostbyname("localhost");
    if(server_info == NULL){
        fprintf(stderr,"otp_dec: Invalid host\n");
        exit(1);
    }
    //copy in address to struct for otp_dec_d
    ...
    if(socket_fd < 0){
        perror("otp_dec listening socket set up error: ");
        exit(1);
    }

    //connect to otp_dec_d
    if(connect(socket_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0){
        perror("otp_dec error connecting to server: ");
        exit(1);
    }

    //send verifying information to otp_dec_d
    SendMsg(socket_fd, "OTP_DEC");
    //receive response from otp_dec_D
    ...
    if(text_size > 0){
        //send size of plaintext file to otp_dec_d
    ...
    //otp_dec_d confirms size of plaintext file to be sent
    //recv_size = RecvSize(socket_fd);
    if(RecvSize(socket_fd) == text_size){
        SendTextfile(socket_fd, plaintext, text_size);
    }
    else{
        fprintf(stderr, "otp_dec_d did not return correct file size\n");
        close(socket_fd);
        exit(1);
    }
    //receive message from otp_dec_d confirming receipt of file contents
    msg_buffer = RecvMsg(socket_fd);
    if(strcmp(msg_buffer, "success") == 0){
        free(msg_buffer);
    }
    ...
    if(text_size > 0){
        //send size of keytext file to otp_dec_d
        SendSize(socket_fd, &text_size);
    }
    else{
        fprintf(stderr, "Invalid file size\n");
    ...
    //otp_dec_d confirms size of keytext file to be sent
    //recv_size = RecvSize(socket_fd);
    if(RecvSize(socket_fd) == text_size){
    ...
    //recv confirmation of size from otp_dec_d
        //if correct
            //send plaintext
        //else
            //close connection
